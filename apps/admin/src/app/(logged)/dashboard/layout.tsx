...
export default function Layout({
    ...
    const tail = path?.split(head).pop() || "/";
    const tailWihtoutSlash = tail.slice(1);

    const routesLink = routes.filter((route) => route.type === "link") as RouteLink[];

    const route = routesLink.find((route) => (route?.path ? tail.startsWith(route.path) : false));
    if (route) {
        route.active = true;
    }
        ...
        <div className="grid min-h-screen w-full lg:grid-cols-[280px_1fr]">
                            ...
                            {routes.map((route, index) => {
                                ...
                                return (
                                    <Link
                                        ...
                                        href={(route.path ? head + route.path : route.url) as string}
                                    >
            ...
            <div className="flex flex-col max-h-screen  ">
                    ...
                    <div className="flex-1">
                        <Breadcrumbs className="w-fit">
                            {tailWihtoutSlash.split("/").map((crumb, index, array) => {
                                ...
                                return (
                                    <Link className="bg-slate-200 py-1 px-2 rounded-full" href={head + "/" + path} key={index}>
                                        {crumb}
                                    </Link>
                                );
                            })}
                ...
                <main className="flex flex-1 flex-col gap-4 h-0">
                    <div className="p-4 md:gap-8 md:p-6 h-full overflow-y-scroll relative">{children}</div>
                </main>
            </div>
        </div>
    );
