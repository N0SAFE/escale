...
import { getSession, login as _login, whoami, cookiesGetAll, logout as _logout, Session } from "@/lib/auth";
...
export default function Home() {
    ...
    const login = async () => {
        console.log(await _login("sssebillemathis@gmail.com", 'SebilleMat3103*'))
    };
    const logout = async () => {
        console.log(await _logout())
    }
    return (
        <main className="flex min-h-screen flex-col items-center justify-between p-24">
            <div className="z-10 max-w-5xl w-full items-center justify-between font-mono text-sm lg:flex">
                ...
                <div className="flex flex-col gap-4">
                    <span>sessions {!!session ? "exist" : "not exist"}</span>
                    <span>user is {!!session?.isLoggedIn ? "connected" : "not connected"}</span>
                </div>
