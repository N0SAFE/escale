...
>({ onSort, onSortStart, onSortEnd, onUpload, onUploadStart, onUploadEnd, renderItem, data, isLoading, className }: React.HtmlHTMLAttributes<HTMLDivElement> & FileUploaderProps<D>) {
    ...
    async function handleOrderChange(array: D[]) {
        onSortStart?.();
        setImages(array);
        const sorted = array.map((item, index) => ({ ...item, id: (item as any).lastId, order: index + 1 }));
        await onSort?.(sorted)
        onSortEnd?.()
    }
    ...
    return (
        <div className={cn("flex flex-col gap-3", className)}>
            ...
            >
                ...
                <div className="text-sm text-gray-500">Drag and drop your files here</div>
                <input type="file" multiple className="hidden" ref={inputFileRef} onChange={handleFileUpload} />
            </Button>
            {isLoading && !data ? (
