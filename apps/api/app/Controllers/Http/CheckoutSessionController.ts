...
export default class CheckoutSessionsController {
  ...
  public async dayOrNightSpa ({ request, response }: HttpContextContract) {
    ...
    try {
      const res = await this.reservationService.getReservationByDay(body.date, body.spa)

      if (res) {
        return response.badRequest({ message: 'this day is already reserved' })
      }

      const availability = await this.reservationService.getAvailabilitieByDay(body.date, body.spa)
